...
def build_arguments() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate the Python bindings for Frida.")
    p.add_argument(
        "--frida-gir",
        required=True,
        type=Path,
    ...
    p.add_argument(
        "--frida-header",
        type=Path,
        help="Path to the Frida C header; when provided, unavailable GIR types are omitted.",
    )
    p.add_argument(
        "--glib-gir",
        required=True,
    ...
    p.add_argument(
        ...
        required=True,
        type=Path,
        help="Path to the output asyncio facade .py file.",
    )
    p.add_argument(
        "--output-c",
        required=True,
...
def run(args: argparse.Namespace) -> None:
    ...
    with OutputFile(args.output_aio) as f:
        f.write(artefacts["aio"])
