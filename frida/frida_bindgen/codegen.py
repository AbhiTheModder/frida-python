...
LEGACY_ERROR_MEMBERS = [
    ("FRIDA_ERROR_SERVER_NOT_RUNNING", "ServerNotRunning"),
    ("FRIDA_ERROR_EXECUTABLE_NOT_FOUND", "ExecutableNotFound"),
    ("FRIDA_ERROR_EXECUTABLE_NOT_SUPPORTED", "ExecutableNotSupported"),
    ("FRIDA_ERROR_PROCESS_NOT_FOUND", "ProcessNotFound"),
    ("FRIDA_ERROR_PROCESS_NOT_RESPONDING", "ProcessNotResponding"),
    ("FRIDA_ERROR_INVALID_ARGUMENT", "InvalidArgument"),
    ("FRIDA_ERROR_INVALID_OPERATION", "InvalidOperation"),
    ("FRIDA_ERROR_PERMISSION_DENIED", "PermissionDenied"),
    ("FRIDA_ERROR_ADDRESS_IN_USE", "AddressInUse"),
    ("FRIDA_ERROR_TIMED_OUT", "TimedOut"),
    ("FRIDA_ERROR_NOT_SUPPORTED", "NotSupported"),
    ("FRIDA_ERROR_PROTOCOL", "Protocol"),
    ("FRIDA_ERROR_TRANSPORT", "Transport"),
]
...
def error_members(model: Model) -> List[Tuple[str, str]]:
    if model.error_domain is None:
        return LEGACY_ERROR_MEMBERS
    return [(member.c_identifier, member.js_name) for member in model.error_domain.members]
...
def generate_py_exception_reexports(model: Model) -> List[str]:
    names = [f"{name}Error" for _, name in error_members(model)]
...
def generate_extension_pyi(model: Model) -> str:
    ...
    for _, name in error_members(model):
        lines.append(f"class {name}Error(Exception): ...")
...
def generate_init_function(model: Model) -> str:
    ...
    exception_registration = indent_c_code(generate_exception_registration(model), 1, prologue="\n")
...
def generate_exception_registration(model: Model) -> str:
    lines = ["frida_exception_by_error_code = g_hash_table_new_full (NULL, NULL, NULL, PyFrida_object_decref);"]
    lines += [
        f'PYFRIDA_DECLARE_EXCEPTION ({c_identifier}, "{name}");' for c_identifier, name in error_members(model)
    ]
    lines += [
